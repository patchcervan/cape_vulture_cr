...
int4 <- list(dist3 ~ age_fct(ad), mov ~ ttnoon, mov ~ ttnoon_sq) # basic interactions with log dist
...
models <- list(
    ...
    # mod4 = form(vults, mod_elem, c("dist", "topo2", "hab", "mov"), intr = c(int1, list(mov ~ res, topo2 ~ zone_fct(z_1), hab ~ zone_fct(z_1)))),
    ...
    mod11 = form(vults, mod_elem, c("dist3", "topo2", "hab", "mov"), intr = c(int4, list(mov ~ res, topo2 ~ zone_fct(z_1))))
)
...
source("R/functions/validateCVssf.R")

for(m in seq_along(models)){
    
    cv_results <- future_map(cv_ids, ~validateCVssf(train_data = "data/working/data_ssf_ready.rds",
                                               test_data = "data/working/data_ssf_ready.rds",
                                               test_ids = .x, model = models[[m]], coef_dir = "output/cv_coef/",
                                               seed = 5412),
                             .options = furrr_options(seed = 5412))
    
    cv_results <- do.call("rbind", cv_results)
    
    # Save cross-validation results
    saveRDS(cv_results, paste0("output/cv_results_", m, "_new.rds"))
}
...
# Load from cluster
files <- list.files(resultsdir, pattern = "cv_results_[1-5]_new.rds")
...
cv_results %>% 
    filter(!is.na(AIC)) %>% 
    dplyr::select(-c(formula, AIC, BIC, logLik)) %>%
    ggplot() +
    geom_boxplot(aes(x = mod, y = cv_pi)) +
    geom_jitter(aes(x = mod, y = cv_pi), col = "red", alpha = 0.5, width = 0.1)
# Warning messages are fine it is that the columns inherited attributes from previous
# objects

ggsave("figures/step_val_allres.png")

cv_results %>% 
    filter(test == "cv7")
    ggplot() +
    geom_point(aes(x = mod, y = cv_pi, col = test), alpha = 0.5) +
    geom_line(aes(x = mod, y = cv_pi, col = test, group = test), alpha = 0.5)
# Warning messages are fine it is that the columns inherited attributes from previous
# objects
