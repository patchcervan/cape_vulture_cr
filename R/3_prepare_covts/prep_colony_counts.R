...
avg_counts <- avg_counts %>% 
   mutate(ever = if_else(rowSums(avg_counts[,2:8], na.rm = T) == 0, 0, 1))
...
sort(unique(avg_counts$name))
sort(unique(col_locs$name))

# Join counts and locations -----------------------------------------------

col_data <- left_join(avg_counts,
...
matches <- cbind(avg_counts$name_new, rep(NA, nrow(avg_counts)))
...
avg_counts <- left_join(avg_counts,
                        dplyr::select(col_locs, id, name, lon, lat, type),
                        by = c("name_new" = "name"))