...
# We only use the birds that are considered ready for model fit
...
db <- read_csv("data/working/bird_db_fit_ready.csv")


# Explore age categories --------------------------------------------------

# Overall age. Considering only age at capture
age <- db %>% 
    mutate(age = sapply((sapply(db$age, str_split, ",")), "[", 1)) %>% 
    ggplot() +
...
prov_age <- db %>% 
    mutate(age = sapply((sapply(db$age, str_split, ",")), "[", 1),
           provider = substring(bird_id, 1,2)) %>% 
...
ttl <- db %>% 
    ggplot() +
    geom_pointrange(aes(x = med_dt, y = bird_id,
                        xmin = 0,
                        xmax = med_dt + sd_dt,
                        col = age)) +
...
tres <- db %>% 
    ggplot() +
    geom_histogram(aes(x = med_dt), binwidth = 1) +
    stat_bin(aes(x = med_dt), binwidth = 1, col = "white") +
    stat_bin(aes(x = med_dt, y = 0.8*(..count..), label = ..count..), binwidth = 1,
             col = "white", geom = "text", ) +
...
pdf(file = "output/db_fit_ready_exploration.pdf")
