fitMixBatch <- function(trk, fig_path = getwd(), file_path = getwd()){
   ...
   # Annontate night-time locations
   daylight <- trk %>% 
   ...
   # trk_mod will be a working data frame. We keep trk untouched from here. We will need
   # an id to match locations later
   trk <- trk %>% 
   ...
   if(id_sel != "ma07"){ # We know ma07 has no speed information, we deal with it later
      ...
      if(nrow(trk_mod) == 0) stop("No valid locations")
   }
   ...
   print(paste0("median dt = ", median(trk_mod$dt, na.rm = T)))
   ...
   # Select covariates (also keep id_loc!!!)
   trk_mod <- trk_mod %>% 
      dplyr::select(id_loc, spd_h, avg_spd, lon, lat, bird_id, dt, ttnoon, ttnoon_sq,
   ...
   if(median(trk_mod$dt, na.rm = T) < 3 & !(id_sel %in% c("kr01"))){
      ...
      print(paste0("keeping ", 100* nrow(trk_mod) / nrow(trk_mod), "% of data"))
      ...
      trk_mod$state <- states
      
      trk <- left_join(trk,
                       dplyr::select(trk_mod, id_loc, state),
                       by = "id_loc")
      ...
      if(id_sel %in% c("wt10", "wt12", "wt14", "wt15")){
         ...
         trk$state <- NA
         trk$state[trk$spd_h <= 6 & trk$day == 1] <- 1
         trk$state[trk$spd_h > 6 & trk$day == 1] <- 2
      } else {
         ...
         trk$state <- NA
         trk$state[trk$spd_h == 0 & trk$day == 1] <- 1
         trk$state[trk$spd_h > 0 & trk$day == 1] <- states_df$state
