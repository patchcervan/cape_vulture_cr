calcHgtRisk <- function(.mapcode, .col_to_pred = NULL, .age, scale = FALSE, .rasterdir, .outputdir){
      ...
      colfiles <- list.files(paste0(.rasterdir, "hgt_gam"))
      ...
      if(!scale){
         ...
         for(i in seq_along(colfiles)){
            ...
            ud_col <- readRDS(paste0(.rasterdir, "hgt_gam/", colfiles[i]))
         ...
         gamfile <- paste0(.outputdir, "hgt_gamfit_", .mapcode, "_", .age, "_nosc", ".tif")
         ...
         gamfile <- paste0(.outputdir, "hgt_gamfit_", .mapcode, "_", .age, ".tif")
