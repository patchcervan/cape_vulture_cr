simTrips <- function(.nsteps=nsteps, .trip_dur=trip_dur, .age=age, .col_sel=col_sel,
   ...
   require(Rcpp)   
      ...
      for(t in 1:15){
         X[[t]] <- .hab %>% 
            mutate(across(.cols = ends_with("ttnoon"), ~.x*c(-7:7)[t]),
                   across(.cols = ends_with("ttnoon_sq"), ~.x*(c(-7:7)^2)[t])) %>% 
      ...
      for(j in 1:.nsteps){
            ...
            sls_correct <- cbind(sls, sls*ttnoon, sls*ttnoon_sq, sls) %*% sl_coefs
            ...
            if(ttnoon > 7){
                  ...
                  if(!any(sims$dist_col[(j-15):j] < 5000)){
                        pback <- pnbinom(floor(dur/15), size = nbinom_dur["size"], mu = nbinom_dur["mu"])
                        if(rbinom(1, 1, pback)){
                              state <- c(.col_sel$lon, .col_sel$lat)
                              state_proj <- c(0,0)
                              dur <- 0
