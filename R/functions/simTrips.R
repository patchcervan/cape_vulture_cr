simTrips <- function(.nsteps=nsteps, .trip_dur=trip_dur, .age=age, .col_sel=col_sel,
   ...
   for(t in 1:15){
      ...
      X[[t]] <- .hab %>% 
         ...
         dplyr::select(all_of(names(.ssf_coef))) %>% 
         as.matrix() %*% .ssf_coef
   }
   ...
   pback <- pnbinom(0:15, size = nbinom_dur["size"], mu = nbinom_dur["mu"])
   ...
   for(j in 1:.nsteps){
      ...
      stephab <- .hab %>% 
         mutate(.sls = sls,
                predh = X[[t]]) %>%
         filter(.sls < 150000) %>% 
         mutate(wgamma = dgamma(.sls, shape = .mov_ker["shape"], scale = .mov_ker["scale"])/(.sls*0.0063)) # See the script sl_dgamma_correct (there aren't 2pi cells per distance)
