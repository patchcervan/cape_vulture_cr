predSelFromCol_SE <- function(map_code, colony, model, age, covts, output_res,
                           colony_all = colony_all, sfs = sfs, se = FALSE, rasterdir){
   
   # Load function to make predictions with standard errors using delta method
   source("tests/functions/predictSE.R")
   ...
   if(se == TRUE){
      pred_sel <- predictSE(mod = model, newdata = rr_df, se.fit = TRUE, level = 0, multicore = NULL)
