...
for(i in 1:length(trk_files)){
   ...
   # Change names to be consistent with the rest of the script and make a spatial object
   trk_sel <- trk_sel %>% 
      as.data.frame() %>% 
      rename(lon = x_,
             lat = y_,
   ...
   saveRDS(trk_sel, paste0("data/working/bird_tracks/fit_ready/height/test_", id_sel,".rds"))
...
trkfiles <- list.files("data/working/bird_tracks/fit_ready/height", pattern = "test")
...
saveRDS(model_data, "data/working/data_height_test.rds")


# Check temporal coverage -------------------------------------------------

vults <- readRDS("data/working/data_height_test.rds")
