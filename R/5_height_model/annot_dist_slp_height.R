...
calcDistances <- function(x, slp){
   ...
   trk <- SpatialPointsDataFrame(coords = x[,c("lon", "lat")], data = x,
   ...
   trk <- spTransform(trk, CRSobj = CRS(tmerproj))
   ...
   mindist <- minDist_cpp(coordinates(trk), coordinates(slp_sel))
...
saveRDS(vults, "data/working/data_height_ready.rds")
