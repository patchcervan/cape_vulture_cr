...
# In this script we fit a RSF model to a single vulture around its colony
...
max_dist <- quantile(st_distance(colony, trk), 0.99)
    
# Define available area
av_area <- st_buffer(x = colony, dist = max_dist)
...
# Create file name
saveas <- paste0("output/rsf_colony_fit_", unique(trk$bird_id), ".rds")
